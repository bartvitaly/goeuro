...
public class SearchPage extends WebDriverCommon {
	...
	public void verifySearch(RoutePlan routePlan) throws InterruptedException {
		...
		checkAttribute(departureDate, "value", Common.formatDate(routePlan.departureDate, "dd/MM/yyyy"), true);
		checkAttribute(returnDate, "value", Common.formatDate(routePlan.returnDate, "dd/MM/yyyy"), true);
		
		// Check persons count
		String personCountActual = getText(personCounter);
		String personCountExpected = String.valueOf(IntStream.of(routePlan.personCount).sum());
		if (!personCountActual.equals(personCountExpected)) {
			logger.error("Person count is incorrect, expected '" + personCountExpected + "', actual '" + personCountActual + "'");
		}

		// Check round or one way trip
		click_wait(searchFromMore);
		if (routePlan.roundTrip) { checkAttribute(roundTrip, "class", "active", true); }
		else { checkAttribute(oneWay, "class", "active", true); }
		
		// Check persons count
		checkAttribute(personCounterAdults, "value", routePlan.personCount[0], true);
		checkAttribute(personCounterChildren, "value", routePlan.personCount[1], true);
		checkAttribute(personCounterInfants, "value", routePlan.personCount[2], true);
