...
/**
 * This class describes a home page of the web site and page elements
 * 
 * @author bartvitaly
 * @version 1.1
 */
public class HomePage extends WebDriverCommon {
	...
	By datepickerDay = By
			.cssSelector(".ui-datepicker-calendar [data-handler='selectDay']");
	...
	By accomodationCheckbox = By
			.cssSelector(".hotel-checkbox:not(.hidden) label");
	...
	public void fillSearchData(RoutePlan routePlan) {
		...
		for (WebElement element : allElements) {
			WebElement accommodationCheckbox = element.findElement(By
					.cssSelector("span"));
			if (routePlan.accomodation == null
					&& checkAttribute(accommodationCheckbox, "class",
							"checked", false)) {
				click(accommodationCheckbox);
				continue;
			}
			if (Arrays.asList(routePlan.accomodation).contains(
					element.getText())
					&& checkAttribute(accommodationCheckbox, "class",
							"checked", false)) {
	...
	public void setDate(DateTime date) {
		...
		String monthLocal = new SimpleDateFormat("MMMM", Common.getLocale())
				.format(new Date(date.getMillis()));
		while (!(Integer.parseInt(getText(datepickerYear)) == date.getYear())
				|| !getText(datepickerMonth).equalsIgnoreCase(monthLocal)) {
	...
	/**
	 * This method executes fillSearchData and clicks search button it closes
	 * any other tabs that are opened after clicking Search button
	 * 
	 * @param routePlan
		...
		if (accommodation != null && accommodation.length > 0
			...
			for (int i = 0; i < accommodation.length; i++) {
				for (int j = 0; j < tabs.size(); j++) {
					String title = driver.switchTo().window(tabs.get(j))
							.getTitle();
					if (title.toLowerCase().contains(
							accommodation[i].toLowerCase())) {
						driver.close();
						j = 0;
